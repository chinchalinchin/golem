...
* **`brain.working_memory` ()**: Dictates the sheer capacity of the agent's temporal state tensor.
* **`training.sequence_length` ()**: Dictates the temporal window (number of contiguous frames) passed into the network during a single Backpropagation Through Time (BPTT) step.

### Tuning Heuristics

* **The Amnesia Trap:** If  is too short (e.g., 8 frames), the network cannot reliably link cause and effect (e.g., observing a projectile being fired and the subsequent impact). The gradients are truncated before the ODE can establish a meaningful time constant. **Always maintain  for combat scenarios.**
* **The Capacity Bottleneck:** If you increase  to capture longer causal chains (e.g., ), you must ensure  is large enough to encode that much history. Pushing 64 frames of dense combat data into a tiny  core will result in catastrophic state overwriting. As a rule of thumb, if you double , consider increasing  to prevent bottlenecking.
...
* **`brain.cortical_depth` ()**: Determines the number of convolutional layers. Higher values aggressively pool the tensor into a denser latent representation .
* **`training.batch_size` and `training.learning_rate**`: Govern how the Adam optimizer navigates the gradient manifold.

### Tuning Heuristics

* **Scaling the Visual Cortex:** A shallow network () is sufficient for basic navigation but fails to resolve distant enemies in complex WADs. Increasing to  allows the network to comprehend complex geometry. However, adding layers increases the parameter count, which generally requires more `epochs` to converge.
* **The Linear Scaling Rule:** Hardware VRAM is your primary constraint. If increasing  or  causes an Out-Of-Memory (OOM) error, you must reduce the `batch_size`. If you halve your `batch_size` (e.g., from 32 to 16), the gradient estimates become noisier. To prevent the optimizer from over-correcting on this noise, you should generally halve your `learning_rate` (e.g., from  to ).
...
* **Thermal Impact (`brain.sensors.thermal`)**: The thermal mask adds an isolated, parallel CNN. Because it is a binary mask (), it is computationally lightweight compared to visual or auditory tensors. You can generally enable this without drastically reducing your batch size.
...
2.  Decrease `loss.asymmetric.gamma_pos` (e.g., to ) to ensure the network receives the full, unmitigated gradient whenever a rare "Attack" label appears in the dataset.
3.  Increase `loss.asymmetric.gamma_neg` (e.g., to ) to aggressively silence the gradients of easily predicted background frames.
...
2. Set `loss.smooth.epsilon` to  or . This injects a uniform noise prior into the targets, acknowledging that human demonstrators are noisy and reactive. It prevents the logits from being pushed to extreme asymptotes, resulting in smoother, more deliberate decision-making.
